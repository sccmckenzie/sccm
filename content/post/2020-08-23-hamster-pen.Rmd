---
title: Corralling Your Pet Hamster
description: FiveThirtyEight's Riddler Puzzle
...
<span style = "background: #0E9AA7; color: white;">The answer to this riddle is $`r roots(3)[1]`$.</span>
...
```{r k-crossover, echo = FALSE, fig.cap = "Each point represents the *highest* value of $k$ where one would wish to maintain current shape before reducing $n$ by $1$", fig.align="center"}
k_range <- tibble(n = 3:10) %>% 
  mutate(k_max = 1/n)
  

k_crossover %>% 
  ggplot() +
  geom_segment(aes(x = n, xend = n, y = 0, yend = k_max), data = k_range) +
  geom_point(aes(n, k), color = "#E84A5F", size = 3) +
  scale_x_continuous(breaks = 3:10) +
  theme(panel.grid.minor = element_blank()) +
  labs(title = "Shape Threshold Values", subtitle = "Points superimposed on allowable ranges for k", y = "")
```
